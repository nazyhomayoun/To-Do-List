...
def create_task_cli(service: ToDoListService, project_id: uuid.UUID):
    ...
    # گرفتن ورودی ددلاین و اعتبارسنجی قالب آن
    ...
    try:
        ...
        # اگر parse_deadline خطا دهد (فرمت تاریخ نادرست)
...
# main.py (توابع task_operation_menu و task_menu)
...
def task_operation_menu(service: ToDoListService, project_id: uuid.UUID):
    ...
    # فرض می‌کنیم get_project_details در سرویس وجود دارد تا نام پروژه را بگیرید
...
def task_menu(service: ToDoListService):
    ...
    while True:
        # نمایش لیست پروژه‌ها برای انتخاب توسط کاربر
        projects = list_projects(service) 
        if not projects:
        ...
        try:
            ...
            # اگر پروژه انتخاب شد، به منوی عملیات تسک می‌رویم
            task_operation_menu(service, project_id)
            # پس از بازگشت از منوی عملیات، دوباره منوی انتخاب پروژه را نشان می‌دهیم
            # اگر می‌خواهید مستقیما به منوی اصلی برگردید، از 'break' استفاده کنید.
